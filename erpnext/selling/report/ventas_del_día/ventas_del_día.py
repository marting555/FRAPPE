...
def execute(filters=None):
	...
	columns = [_("Fecha") + "::240", _("Serie") + "::240", _("Rango") + "::240", _("Importe bruto") + ":Currency:120", _("Ventas exentas") + ":Currency:120", _("Ventas gravadas 15%") + ":Currency:120", _("I.S.V 15%") + ":Currency:120", _("Ventas gravadas 18%") + ":Currency:120", _("I.S.V 18%") + ":Currency:120",_("Monto descuento") + ":Currency:120", _("Total") + ":Currency:120"]
...
def return_data(filters):
	...
	salary_slips = frappe.get_all("Sales Invoice", ["name", "status","naming_series", "posting_date", "authorized_range", "exempt_amount", "taxed_amount_15", "isv_15", "taxed_amount_18", "isv_18", "grand_total", "discount_amount", "total"], filters = conditions,  order_by = "name asc")
	...
	for date in dates_reverse:
		...
		for salary_slip in salary_slips:
			...
			if date == date_validate and salary_slip.status != "Return":
				...
				total_exempt += salary_slip.exempt_amount
		...
		row = [creation_date, serie_final, final_range, gross, total_exempt, taxed_sales15, isv15, taxed_sales18, isv18, discount_amount, grand_total]
		data.append(row)
