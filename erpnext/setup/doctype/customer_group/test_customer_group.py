...
class TestCustomerGroup(unittest.TestCase):
    def setUp(self):
        self.customer_group_name = "_Test Customer Group_1"
    ...
    def tearDown(self):
        try:
            frappe.delete_doc("Customer Group", self.customer_group.name, force=1, ignore_permissions=True)
        except frappe.LinkExistsError:
            frappe.msgprint(
                msg=f"Cannot delete Customer Group '{self.customer_group_name}' because it is linked to other documents.",
                alert=True
            )
