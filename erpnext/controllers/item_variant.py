...
def validate_item_attribute_value(
    ...
    if allow_rename_attribute_value:
        ...
        if from_variant:
            ...
            frappe.throw(
                msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed")
            )
...
def get_attribute_values(item):
    if not frappe.flags.attribute_values:
        ...
        # only load the attributes defined on this template
        attrs = [
            d.attribute
        ...
        # load just the matching Item Attribute Value rows
        for t in frappe.get_all(
            "Item Attribute Value",
            filters={"parent": ["in", attrs]},
            fields=["parent", "attribute_value"],
        ):
            attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

        # numeric‐range settings as before
        for t in frappe.get_all(
            "Item Variant Attribute",
            filters={"parent": item.variant_of, "numeric_values": 1},
